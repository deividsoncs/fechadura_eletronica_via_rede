/**
    Protótipo de Fechadura Eletrônica controlada
...
//usuário Banco
char user[] = "arduino2";
char password[] = "123456"; //Senha será convertida usando sha1 na transmissão!

const char PORTA_ATUACAO[] = "PORTA N";
...
//id do usuario resgatando do banco
long idUsuario = 0;
//SELECT sempre retornara os 16 primeiros caracteres do nome.
char *nomeUsuario = (char *)malloc(16 * sizeof(char));
char *senhaBco = (char *)malloc(4 * sizeof(char));
String strChave = "";
...
/*
  X minutos checar se está conectado e faz um HAND SHAKE no banco
  renovando a conexão evitando que o MySQL de um Kill Connection.
*/
const long TEMPO_RENOVA_CONEX = 3600000; //uma hora 3600000
...
//Utilizado para realizar o ping
SOCKET pingSocket = 1;
...
void loop() {
  ...
  if (millisAtualRenovaConex < millisAnteriorRenovaConex) {
    millisAnteriorRenovaConex = millisAtualRenovaConex;
  }
  ...
  if (millisAtualConex - millisAnteriorConex >= TEMPO_VERIFICA_CONEX) {
    ...
    //Serial.println("----------------");
    //Serial.print("| PING ? ");
    ICMPEchoReply echoReply = ping(servidorSQL, 4);
    delay(30);
    if (echoReply.status == SUCCESS) {
      ...
      if (echoReply.status == SUCCESS) {
        ...
        digitalWrite(BACKLIGHT_LED, HIGH);
        lcd.setCursor(0, 0);
        lcd.write("ERRO SEM PING!!!");
        lcd.setCursor(0, 1);
        lcd.write("BANCO||REDE FORA");
        delay(4000);
        //Uso soft_reset pois reconecta mais rapido do que ficar tentando pelo conectar()
        soft_reset();
      }
    }
  }
  ...
  //##############################################################################################
  //####################################RENOVA CONEXAO MYSQL######################################
  //##############################################################################################
  if (millisAtualRenovaConex - millisAnteriorRenovaConex >= TEMPO_RENOVA_CONEX) {
    selecionaETH();
    millisAnteriorRenovaConex = millisAtualRenovaConex;
    if (my_conn.is_connected()) {
      num_fails = 0;
      Serial.println("-----------------------");
      Serial.println("Renovando conexao MySQL!");
      //Atualiza a conexão para o MySQL não derruba-la
      my_conn.cmd_query("SELECT @@version");
      my_conn.show_results();
      Serial.println("-----------------------");
    } else {
      Serial.println("Resetando");
